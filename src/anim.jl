...
# For Animation
using ..RPG: SDL2, AbstractPos, Camera
# For Sprite
...
loopanim_callback(a, dt) = RPG.update!(reset_anim!(a), dt)

iscompleted(a::Animation) = a.completed

function RPG.render(a::Animation, pos::AbstractPos{C}, cam::Camera, renderer;
                size) where C
    RPG.render(a.frames[a.cur_frame], pos, cam, renderer; size=size)
end
function RPG.update!(a::Animation, dt)
...
function RPG.render(s::Sprite, pos::AbstractPos{C}, cam::Camera, renderer;
                size = nothing) where C
    src_rect=SDL2.Rect(s.pos..., s.dims...)
    RPG.render(s.img, pos, cam, renderer;
               size=size,
